...
Route::prefix('v1')->group(function () {
    ...
    Route::get('test-replace-debug', function() {
        ...
        try {
            ...
            // Let's do a mock test of replaceRecursiveHelper
            echo "Testing replaceRecursiveHelper with Site Settings...\n";
            $testObj = \Illuminate\Support\Facades\DB::table('site_settings')->first();
            if ($testObj) {
                echo "SiteSetting key: " . $testObj->key . "\n";
                // Let's call the controller helper directly!
                $ctrl = new \App\Http\Controllers\Api\Settings\SettingsController();
                $refObj = new ReflectionObject($ctrl);
                $m = $refObj->getMethod('replaceRecursiveHelper');
                $m->setAccessible(true);
                $res = $m->invoke($ctrl, $testObj->value, $currentCompanyName, $currentSiteName, $replace);
                echo "replaceRecursiveHelper succeeded on key: " . $testObj->key . "\n";
            }
    ...
    Route::middleware(['auth:sanctum'])->group(function () {
        ...
        Route::get('orders/export', [\App\Http\Controllers\Api\OrdersController::class, 'adminExport'])->middleware('admin');
        Route::post('orders/import', [\App\Http\Controllers\Api\OrdersController::class, 'adminImport'])->middleware('admin');
