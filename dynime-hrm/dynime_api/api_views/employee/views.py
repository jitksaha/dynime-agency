...
from dynime.decorators import owner_can_enter
from dynime_api.api_decorators.base.decorators import permission_required
from dynime_api.api_methods.employee.methods import get_next_badge_id
...
class DocumentRequestAPIView(APIView):
    ...
    @manager_permission_required("dynime_documents.add_documentrequests")
    def post(self, request):
    ...
    @manager_permission_required("dynime_documents.change_documentrequests")
    def put(self, request, pk):
        document_request = self.get_object(pk)
...
class DocumentAPIView(APIView):
    ...
    def get(self, request, pk=None):
        employee = request.user.employee_get
        has_view_perm = request.user.has_perm("dynime_documents.view_document")

        if pk:
            document = self.get_object(pk)
            if not can_access_document(
                request, document, "dynime_documents.view_document"
            ):
                return Response(
    ...
    @manager_or_owner_permission_required(
        DocumentRequest, "dynime_documents.add_document"
    )
    def post(self, request):
    ...
    @method_decorator(owner_can_enter("dynime_documents.change_document", Employee))
    def put(self, request, pk):
        document = self.get_object(pk)
    ...
    @method_decorator(owner_can_enter("dynime_documents.delete_document", Employee))
    def delete(self, request, pk):
        document = self.get_object(pk)
        document.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class DocumentRequestApproveRejectView(APIView):
    ...
    @manager_permission_required("dynime_documents.add_document")
