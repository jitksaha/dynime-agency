...
from dynime import dynime_apps
from dynime.decorators import (
    delete_permission,
...
from dynime.group_by import group_by_queryset
from dynime.dynime_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from dynime.methods import get_dynime_model_class, remove_dynamic_url
from dynime_audit.forms import HistoryTrackingFieldsForm
from dynime_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/dynime_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/dynime_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/dynime_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/dynime_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/dynime_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/dynime_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/dynime_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/dynime_job_position.html",
...
def two_factor_auth(request):
    ...
    if not dynime_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_dynime_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(dynime_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/dynime-logo.png")
...
@login_required
@permission_required("base.view_dynimemailtemplate")
...
@login_required
@hx_request_required
@permission_required("base.change_dynimemailtemplate")
...
@login_required
@hx_request_required
@permission_required("base.add_dynimemailtemplate")
...
@login_required
@permission_required("base.delete_dynimemailtemplate")
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_dynime_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_dynime_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_dynime_model_class(
...
@login_required
@permission_required("dynime_audit.change_accountblockunblock")
...
@login_required
@permission_required("dynime_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("dynime_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("dynime_audit.change_audittag")
...
@login_required
def get_dynime_installed_apps(request):
