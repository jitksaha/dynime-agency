...
from dynime.filters import HorillaPaginator
from dynime.group_by import group_by_queryset
from dynime.dynime_settings import HORILLA_DATE_FORMATS
from dynime.http import HorillaRedirect
from dynime.methods import get_dynime_model_class
from dynime_audit.models import AccountBlockUnblock, HistoryTrackingFields
from dynime_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
@manager_can_enter("dynime_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="dynime_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("dynime_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "dynime_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "dynime_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("dynime_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("dynime_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("dynime_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("dynime_documents.delete_document"):
...
def file_upload(request, id):
    ...
    if not can_access_document(
        request, document_item, "dynime_documents.change_document"
    ):
        return HorillaRedirect(
...
def view_file(request, id):
    ...
    if not can_access_document(
        request, document_obj, "dynime_documents.view_document"
    ):
        return HorillaRedirect(
...
@login_required
@hx_request_required
@manager_can_enter("dynime_documents.add_document")
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_dynime_model_class(
                    ...
                    OnboardingStage = get_dynime_model_class(
                    ...
                    OnboardingTask = get_dynime_model_class(
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_dynime_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_dynime_model_class(
