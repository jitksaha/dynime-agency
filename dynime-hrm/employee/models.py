...
from base.dynime_company_manager import HorillaCompanyManager
...
from dynime import dynime_middlewares
from dynime.methods import get_dynime_model_class
from dynime.models import HorillaModel, has_xss, upload_path
from dynime_audit.methods import get_diff
from dynime_audit.models import HorillaAuditInfo, HorillaAuditLog
...
class Employee(models.Model):
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_dynime_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_dynime_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_dynime_model_class("recruitment", "recruitment")
            Stage = get_dynime_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_dynime_model_class("attendance", "attendance")
            request = getattr(dynime_middlewares._thread_locals, "request", None)
