...
from dynime.dynime_settings import BIO_DEVICE_THREADS
from dynime.http.response import HorillaRedirect
from dynime.settings import TIME_ZONE
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_dynime_cosec_user(request, user_id, device_id):
    """
...
def map_biometric_users(request, device_id):
    """
    Maps an dynime employee to a biometric user on a specified biometric device.
    """
    device = BiometricDevices.find(device_id)
