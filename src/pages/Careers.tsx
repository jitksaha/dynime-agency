...
const JobDetailModal = ({ job, onClose }: { job: SyncedJob; onClose: () => void }) => {
  ...
  return (
    ...
    >
      <div className="relative bg-background border border-border/50 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in slide-in-from-bottom-4 duration-300">
        {/* Modal Header */}
        <div className="flex items-start justify-between gap-4 p-6 pb-4 border-b border-border/20 shrink-0">
          <div className="flex gap-4 items-start flex-1 min-w-0">
            <div className="w-14 h-14 bg-white border border-border/40 rounded-xl flex items-center justify-center shrink-0 overflow-hidden p-1.5">
        ...
        {/* Modal Footer */}
        <div className="shrink-0 p-4 border-t border-border/20 flex items-center justify-between gap-3 bg-muted/20 rounded-b-2xl">
          <p className="text-xs text-muted-foreground font-medium">
            Ready to join the team? Click Apply Now to submit your application.
          </p>
          <Button
            onClick={handleApplyRedirect}
            className="font-bold bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl px-6 h-9 text-sm shadow-sm shrink-0"
          >
            Apply Now <ArrowUpRight className="w-3.5 h-3.5 ml-1" />
          </Button>
        </div>
      </div>
    </div>
...
const JobCard = ({ job, onViewDetails }: { job: SyncedJob; onViewDetails: () => void }) => {
  ...
  return (
    <div className="group bg-background border border-border/50 rounded-2xl p-5 hover:border-primary/30 hover:shadow-md transition-all duration-300 flex flex-col gap-4">
      {/* Card Top */}
      <div className="flex items-start gap-3.5">
        <div className="w-12 h-12 bg-white border border-border/40 rounded-xl flex items-center justify-center shrink-0 overflow-hidden p-1.5 shadow-sm">
          <JobBrandLogo className="w-full h-full" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="inline-flex items-center gap-0.5 text-[9px] text-emerald-600 font-bold bg-emerald-500/8 px-1.5 py-0.5 rounded-full border border-emerald-500/15 shrink-0 whitespace-nowrap">
      ...
      <div className="flex flex-wrap gap-1.5">
        {job.location && (
          <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-muted/70 text-muted-foreground border border-border/40">
      ...
      {job.description && (
        <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">
          {job.description.replace(/[#*_`\[\]()]/g, "").trim().slice(0, 140)}…
        </p>
      )}
      ...
      <div className="flex items-center justify-between pt-1 mt-auto">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-bold text-primary hover:text-primary/80 bg-primary/8 hover:bg-primary/15 px-3 py-1.5 rounded-lg transition-all border border-primary/20 hover:border-primary/40"
        >
...
const Careers = () => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      if (search.trim()) {
        ...
        if (!job.title?.toLowerCase().includes(q) && !job.department?.toLowerCase().includes(q) && !job.description?.toLowerCase().includes(q))
          return false;
      }
      return true;
    });
  ...
  const handleResetFilters = () => {
    setSearch(""); setDepartment(""); setEmploymentType(""); setExperience(""); setRemoteOnly(false);
  };
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-slate-50/50 dark:bg-background">
        ...
        {/* ── Hero Banner ─────────────────────────────────────────────── */}
        <section className="relative overflow-hidden bg-gradient-to-b from-primary/5 via-background to-slate-50/50 dark:from-primary/8 dark:via-background dark:to-background border-b border-border/30">
          {/* Decorative background */}
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-primary/5 rounded-full blur-3xl" />
            <div className="absolute top-8 left-16 w-64 h-64 bg-violet-500/5 rounded-full blur-3xl" />
            <div className="absolute top-8 right-16 w-64 h-64 bg-blue-500/5 rounded-full blur-3xl" />
          </div>

          <div className="relative max-w-4xl mx-auto px-4 py-14 text-center">
            {/* Badge */}
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold bg-primary/10 text-primary border border-primary/20 mb-4">
              <Briefcase className="w-3 h-3" /> We're hiring globally
            </span>

            <h1 className="font-heading text-4xl md:text-5xl font-extrabold tracking-tight text-foreground mb-3">
              Find your dream job
            </h1>
            <p className="text-muted-foreground text-sm md:text-base font-medium max-w-xl mx-auto mb-8">
              Work with an ambitious, global, remote-first team building world-class digital products powered by AI.
            </p>

            {/* ── Work Info Pill Cards ──────────────────────────── */}
            <div className="flex flex-wrap justify-center gap-2.5 mb-8">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-left shadow-sm">
                <Timer className="w-4 h-4 text-amber-500 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-amber-700 dark:text-amber-400 block leading-tight">Work Time: 8 Hours</span>
                  <span className="text-[9px] text-amber-600/70 dark:text-amber-400/60 font-medium">Flexible on discussion</span>
                </div>
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-left shadow-sm">
                <Wifi className="w-4 h-4 text-blue-500 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-blue-700 dark:text-blue-400 block leading-tight">Work Type: Remote</span>
                  <span className="text-[9px] text-blue-600/70 dark:text-blue-400/60 font-medium">Work from anywhere</span>
                </div>
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-left shadow-sm">
                <Sun className="w-4 h-4 text-emerald-500 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 block leading-tight">Holiday: Sunday</span>
                  <span className="text-[9px] text-emerald-600/70 dark:text-emerald-400/60 font-medium">You can choose by availability</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 text-center">
              {[
                ...
                <div key={label} className="flex flex-col items-center gap-0.5">
                  ...
                  <span className="font-heading font-extrabold text-lg text-foreground">{value}</span>
        ...
        {/* ── Culture & AI Tags ─────────────────────────────────────── */}
        <div className="max-w-6xl mx-auto px-4 md:px-8 pt-5 pb-1">
          <div className="flex flex-wrap gap-2 items-center">
            {[
              { icon: Bot, label: "AI-Assisted Workflows", cls: "bg-violet-50 dark:bg-violet-500/10 border-violet-200 dark:border-violet-500/20 text-violet-700 dark:text-violet-400" },
              { icon: Sparkles, label: "AI Tools Provided", cls: "bg-pink-50 dark:bg-pink-500/10 border-pink-200 dark:border-pink-500/20 text-pink-700 dark:text-pink-400" },
              { icon: Zap, label: "Fast-Growth Environment", cls: "bg-sky-50 dark:bg-sky-500/10 border-sky-200 dark:border-sky-500/20 text-sky-700 dark:text-sky-400" },
              { icon: Globe, label: "Global Remote Team", cls: "bg-teal-50 dark:bg-teal-500/10 border-teal-200 dark:border-teal-500/20 text-teal-700 dark:text-teal-400" },
              { icon: Heart, label: "Inclusive Culture", cls: "bg-orange-50 dark:bg-orange-500/10 border-orange-200 dark:border-orange-500/20 text-orange-700 dark:text-orange-400" },
              { icon: Shield, label: "Verified Employer", cls: "bg-indigo-50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-400" },
            ].map(({ icon: Icon, label, cls }) => (
              <div key={label} className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border ${cls}`}>
        ...
        {/* ── Filters ───────────────────────────────────────────────── */}
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-4 sticky top-0 z-20 bg-slate-50/90 dark:bg-background/90 backdrop-blur-sm border-b border-border/20">
          <div className="bg-background rounded-xl border border-border/40 p-3 shadow-sm flex flex-col sm:flex-row gap-3 items-center">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              {(search || department || employmentType || experience || remoteOnly) && (
                <button onClick={handleResetFilters} className="text-primary text-xs font-bold hover:underline shrink-0 whitespace-nowrap">
                  Reset
                </button>
              )}
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-8">
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-48 bg-muted/40 animate-pulse rounded-2xl border border-border/40" />
              ))}
            </div>
          ) : isError ? (
            <div className="text-center py-20 bg-background rounded-2xl border border-border/50">
              ...
              <p className="text-muted-foreground text-sm mt-1">Please refresh the page to reload the listing.</p>
            </div>
